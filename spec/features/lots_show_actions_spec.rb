...
RSpec.describe 'Lot show actions', type: :feature, js: true do
  ...
  let(:lot_uuid) { '11111111-2222-3333-4444-555555555556' }
  ...
  it 'creates qcables from the lot show page' do
    ...
    printer = Sequencescape::Api::V2::BarcodePrinter
              .new(print_service: 'PMB', name: 'Test Printer',
                   barcode_type: 'something', uuid: 'baac0dea-0000-0000-0000-000000000000')
  ...
  describe 'Tag Plates card' do
    it 'shows printers from the config file in the Barcode Printer dropdown' do
      stub_lot_show(
        lot_uuid:,
        lot_number: 'PST-12345',
        lot_type_name: 'Pre Stamped Tags'
      ...
      within('.barcode-printing-form') do
        expect(page).to have_select('barcode_printer',
                                    options: %w[plate_example tube_example tef_plate_example])
      end
    end
